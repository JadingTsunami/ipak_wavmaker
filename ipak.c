...
int main( int argc, char** argv ) { 
    ...
    if( argc != 2 ) {
        ...
        fprintf( stderr, "\n" );
        fprintf( stderr, "Unpacks an iPack file to the local directory.\n" );
        return 1;
    }
...
void PK_Init( const char *pakFileName ) {
	...
	for ( int i = 0 ; i < pkHeader->textures.count ; i++ ) {
		pkTextures[i].textureData = (pkTextureData_t *)( (uint8_t *)pkHeader + pkHeader->textures.tableOfs + i * pkHeader->textures.structSize );
        printf("%s\n",pkTextures[i].textureData->name.name);
        int tex = open( pkTextures[i].textureData->name.name, O_WRONLY | O_CREAT );
        if( tex == -1 ) {
            printf( "tga write failed: %s\n", strerror( errno ) );
            assert(0);
        } else {
            write( tex, &pkTextures[i], sizeof(pkTextures[i]) );
            close(tex);
        }
	...
	for ( int i = 0 ; i < pkHeader->wavs.count ; i++ ) {
		...
		sfx->wavData = (pkWavData_t *)( (uint8_t *)pkHeader + pkHeader->wavs.tableOfs + i * pkHeader->wavs.structSize );
		// there is no harm in setting the OpenAl static buffer up for everything now
        //
        printf("%s\n",sfx->wavData->name.name);
        int wav = open( sfx->wavData->name.name, O_WRONLY | O_CREAT );
        if( wav == -1 ) {
            printf( "wav write failed: %s\n", strerror( errno ) );
            assert(0);
        } else {
            printf( "hz: %d, ch: %d, ch-bytes %d, numsamples %d\n", sfx->wavData->wavRate, sfx->wavData->wavChannels,sfx->wavData->wavChannelBytes,sfx->wavData->wavNumSamples);
            printf( "writing %s, %llx offset, %lu bytes\n", sfx->wavData->name.name, (uint8_t *)pkHeader + sfx->wavData->wavDataOfs, (sfx->wavData->wavChannels*sfx->wavData->wavChannelBytes*sizeof(uint8_t)*sfx->wavData->wavNumSamples) );
            write( wav, (uint8_t *)pkHeader + sfx->wavData->wavDataOfs, (sfx->wavData->wavChannels*sfx->wavData->wavChannelBytes*sizeof(uint8_t)*sfx->wavData->wavNumSamples) );
            close(wav);
        }

	}
	int	endLoadingWavs = 0;
...
void PK_LoadTexture( pkTexture_t *tex ) {
	...
	unsigned char *s = (uint8_t *)pkHeader + imd->picDataOfs;
...
/*
 ...
 ==================
 */
const uint8_t *PK_FindRaw( const char *rawName, int *len ) {
	...
	return (uint8_t *)pkHeader + raw->rawDataOfs;
}
...
const pkName_t *PK_FindType( const char *rawName, const pkType_t *type, int *indexOutput ) {
	...
	while ( typeIndex != -1 ) {
		...
		const pkName_t *name = (pkName_t *)((uint8_t *)pkHeader + type->tableOfs + typeIndex * type->structSize );
